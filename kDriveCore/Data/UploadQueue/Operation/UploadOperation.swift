...
public final class UploadOperation: AsynchronousOperation, UploadOperationable, ExpiringActivityDelegate {
    ...
    public required init(uploadFileId: String,
        ...
        UploadOperationLog("init ufid:\(uploadFileId)")
    ...
    override public func execute() async {
        UploadOperationLog("execute \(uploadFileId)")
    ...
    func getUploadSessionOrCreate() async throws {
        ...
        UploadOperationLog("Asking for an upload Session \(uploadFileId)")
    ...
    func generateChunksAndFanOutIfNeeded() async throws {
        UploadOperationLog("generateChunksAndFanOutIfNeeded ufid:\(uploadFileId)")
        try checkCancelation()
        ...
        try transactionWithFile { file in
            ...
            guard let chunkTask = chunksToGenerate.first else {
                UploadOperationLog("generateChunksAndFanOutIfNeeded no remaining chunks to generate ufid:\(self.uploadFileId)")
                return
            }
            UploadOperationLog("generateChunksAndFanOutIfNeeded working with:\(chunkTask.chunkNumber) ufid:\(self.uploadFileId)")
            ...
            guard let chunkProvider = ChunkProvider(fileURL: fileUrl, ranges: [range]),
                ...
                UploadOperationLog("Unable to get a ChunkProvider for \(self.uploadFileId)", level: .error)
            ...
            do {
                ...
                let chunkPath = try self.storeChunk(chunk,
                                                    ...
                                                    hash: chunkSHA256)
                UploadOperationLog("chunk stored count:\(chunkNumber) for:\(self.uploadFileId)")
    ...
    private func scheduleNextChunk(filePath: String, chunksToGenerateCount: Int) async throws {
        do {
            ...
            UploadOperationLog("Unable to schedule next chunk. error:\(error) for:\(uploadFileId)", level: .error)
            throw error
        }
    }
    ...
    func fanOutChunks() async throws {
        ...
        try transactionWithFile { file in
            ...
            guard let uploadingSessionTask: UploadingSessionTask = file.uploadingSession else {
                UploadOperationLog("fanOut no session for:\(self.uploadFileId)", level: .error)
                throw ErrorDomain.uploadSessionTaskMissing
            }
            ...
            UploadOperationLog("fanOut chunksToUpload:\(chunksToUpload.count) freeSlots:\(freeSlots) for:\(self.uploadFileId)")
            ...
            for chunkToUpload: UploadingChunkTask in chunksToUpload {
                ...
                do {
                    ...
                    UploadOperationLog("started task identifier:\(identifier) for:\(self.uploadFileId)")
    ...
    func getFileUrlIfReadable(file: UploadFile) throws -> URL {
        guard let fileUrl = file.pathURL,
            ...
            UploadOperationLog("File has not a valid readable URL:\(file.pathURL?.path) for \(uploadFileId)",
                               level: .error)
    ...
    public func cleanUploadFileSession(file: UploadFile? = nil) {
        UploadOperationLog("Clean uploading session for \(uploadFileId)")
        ...
        let cleanFileClosure: (UploadFile) -> Void = { file in
            ...
            self.enqueueCatching {
                ...
                UploadOperationLog("remove cancelledSession:\(cancelledSession) for \(self.uploadFileId)")

                for (key, value) in self.uploadTasks {
                    UploadOperationLog("cancelled chunk upload request :\(key) ufid:\(self.uploadFileId)")
                    value.cancel()
                }
    ...
    func checkFileIdentity(filePath: String, file: UploadFile? = nil) throws {
        guard fileManager.isReadableFile(atPath: filePath) else {
            UploadOperationLog("File has not a valid readable URL:'\(filePath)' for \(uploadFileId)",
                               level: .error)
            throw DriveError.fileNotFound
        }
    ...
    func checkCancelation() throws {
        if isCancelled {
            UploadOperationLog("Task is cancelled \(uploadFileId)")
            throw ErrorDomain.operationCanceled
        } else if isFinished {
            UploadOperationLog("Task is isFinished \(uploadFileId)")
    ...
    func closeSessionAndEnd() async {
        UploadOperationLog("closeSession ufid:\(uploadFileId)")
        ...
        guard let uploadSessionToken else {
            UploadOperationLog("No existing session to close ufid:\(uploadFileId)")
            return
        }
        ...
        await catching {
            ...
            UploadOperationLog("uploadedFile 'File' id:\(uploadedFile.file.id) ufid:\(self.uploadFileId)")
    ...
    private func storeChunk(_ buffer: Data, number: Int64, uploadFileId: String, sessionToken: String,
        ...
        UploadOperationLog("wrote chunk:\(chunkPath) ufid:\(uploadFileId)")
    ...
    private func getPhAssetIfNeeded() async throws {
        UploadOperationLog("getPhAssetIfNeeded ufid:\(uploadFileId)")
        var assetToLoad: PHAsset?
        try transactionWithFile { file in
            UploadOperationLog("getPhAssetIfNeeded type:\(file.type) ufid:\(self.uploadFileId)")
            guard file.type == .phAsset else {
                return
            }
            guard let asset = file.getPHAsset() else {
                UploadOperationLog("unable to fetch PHAsset ufid:\(self.uploadFileId)", level: .error)
                return
            }
            assetToLoad = asset
        }
        ...
        guard let assetToLoad,
            ...
            UploadOperationLog("Failed to get photo asset ufid:\(uploadFileId)", level: .error)
            return
        }

        // save
        UploadOperationLog("Got photo asset, writing URL:\(url) ufid:\(uploadFileId)")
        try transactionWithFile { file in
            file.pathURL = url
    ...
    public func uploadCompletion(data: Data?, response: URLResponse?, error: Error?) {
        enqueue(asap: true) {
            ...
            if (error != nil) || (statusCode < 200 || statusCode >= 300) {
                ...
                UploadOperationLog("uploadCompletion OK data:\(data?.count) statusCode:\(statusCode) ufid:\(self.uploadFileId)")
            }
    ...
    private func uploadCompletionSuccess(data: Data, response: URLResponse?, error: Error?) async throws {
        UploadOperationLog("completion successful \(uploadFileId)")

        guard let uploadedChunk = try? ApiFetcher.decoder.decode(ApiResponse<UploadedChunk>.self, from: data).data else {
            UploadOperationLog("parsing error:\(error) ufid:\(uploadFileId)", level: .error)
            throw ErrorDomain.parseError
        }
        UploadOperationLog("chunk:\(uploadedChunk.number)  ufid:\(uploadFileId)")
            ...
            if let chunkTask = uploadingSessionTask.chunkTasks.first(where: { $0.chunkNumber == uploadedChunk.number }) {
                ...
                if let path = chunkTask.path {
                    ...
                    DispatchQueue.global(qos: .background).async {
                        UploadOperationLog("cleanup chunk:\(chunkNumber) ufid:\(self.uploadFileId)")
                        try? self.fileManager.removeItem(at: url)
                    }
                }
            } else {
                UploadOperationLog("matching chunk:\(uploadedChunk.number) failed ufid:\(self.uploadFileId)", level: .error)
        ...
        if toUploadCount == 0 {
            enqueue {
                UploadOperationLog("No more chunks to be uploaded \(self.uploadFileId)")
                if !self.isCancelled {
                    await self.closeSessionAndEnd()
                }
            }
    ...
    private func uploadCompletionLocalFailure(data: Data?, response: URLResponse?, error: Error) throws {
        UploadOperationLog("completion Client-side error:\(error) ufid:\(uploadFileId)", level: .error)
        defer {
            self.end()
        }
    ...
    private func uploadCompletionRemoteFailure(data: Data?, response: URLResponse?, error: Error?) {
        ...
        UploadOperationLog("completion  Server-side error:\(error) ufid:\(uploadFileId) ", level: .error)
    ...
    public func end() {
        ...
        defer {
            ...
            UploadOperationLog("call finish \(uploadFileId)")
        ...
        try? transactionWithFile { file in
            if let error = file.error {
                UploadOperationLog("end file:\(self.uploadFileId) errorCode: \(error.code) error:\(error)", level: .error)
            } else {
                UploadOperationLog("end file:\(self.uploadFileId)")
            }

            if let path = file.pathURL,
               file.shouldRemoveAfterUpload && file.uploadDate != nil {
                UploadOperationLog("Remove local file at path:\(path) ufid:\(self.uploadFileId)")
        ...
        if shouldCleanUploadFile {
            UploadOperationLog("Delete file:\(uploadFileId)")
    ...
    public func backgroundActivityExpiring() {
        UploadOperationLog("backgroundActivityExpiring ufid:\(uploadFileId)")
        enqueueCatching(asap: true) {
            try self.transactionWithFile { file in
                file.error = .taskRescheduled
                UploadOperationLog("Rescheduling didReschedule .taskRescheduled ufid:\(self.uploadFileId)")
            ...
            UploadOperationLog("Rescheduling end ufid:\(self.uploadFileId)")
        }
        UploadOperationLog("exit reschedule ufid:\(uploadFileId)")
    }
}
